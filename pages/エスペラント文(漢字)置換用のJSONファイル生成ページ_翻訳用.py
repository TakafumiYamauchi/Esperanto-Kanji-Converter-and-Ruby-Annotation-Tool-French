...
# 
# 【概要】
#  このページでは、「エスペラント文の漢字(フランス語)置換」で使う
#  最終的な JSON ファイル(合并3個JSONファイル)を生成する機能を提供します。
# 
# 【主な流れ】
#  1) CSVファイルを取り込み(エスペラント語根→漢字/フランス語訳の対応)
...
with st.expander("使い方の説明を開く", expanded=True):
    ...
    利用手順は以下の通りです:
    1. 必要な **CSV ファイル**(エスペラント語根→フランス語訳 の対応表など) をアップロード、またはデフォルトを使用。
...
with st.expander("サンプルファイル一覧(ダウンロード用)"):
    ...
    # サンプルCSV１
    st.markdown("""
    **サンプルCSV１(エスペラント語根-フランス語訳ルビ対応リスト)**
    エスペラント語根とフランス語訳を 1 行ずつ対応づけたCSVファイルです。
    ...
    with open(file_path0, "rb") as file:
        btn = st.download_button(
            label="サンプルCSV１(エスペラント語根-フランス語訳ルビ対応リスト)ダウンロード",
            data=file,
            file_name="エスペラント語根-フランス語訳ルビ対応リスト.csv",
            mime="text/csv"
        )
...
if st.button("置換用JSONファイルを作成する"):
    ...
    with st.spinner("置換用JSONファイルを生成中... しばらくお待ちください。"):
        ...
        #     - 後ほどCSVを使って"new"をフランス語訳/漢字訳などに置き換える
        #-------------------------------------------------------------
        ...
        #     「エスペラント語根 → (漢字 or フランス語訳ルビ等)」を反映させる

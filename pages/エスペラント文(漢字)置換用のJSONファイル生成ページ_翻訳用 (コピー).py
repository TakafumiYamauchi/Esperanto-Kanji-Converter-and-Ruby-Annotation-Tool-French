...
# 
# 【概要】
#  このページでは、「エスペラント文の汉字(フランス語)置換」で使う
#  最終的な JSON ファイル(合并3個JSONファイル)を生成する機能を提供します。
# 
# 【主な流れ】
#  1) CSVファイルを取り込み(エスペラント語根→汉字/フランス語訳の対応)
...
#=====================================================================
st.set_page_config(
    page_title="Outil de génération de fichiers JSON pour la substitution (caractères chinois) dans des textes en espéranto",
    layout="wide"
)
st.title("Générer un fichier JSON pour la substitution (caractères chinois) dans des textes en espéranto.")
st.write("---")

#=====================================================================
# 2) 概要説明 (使い方)
#=====================================================================
with st.expander("Ouvrir l'explication de l'utilisation", expanded=True):
    st.markdown("""
    #### Introduction
    Sur cette page, vous pouvez générer et télécharger
    un fichier JSON (qui peut atteindre environ 50Mo)
    utilisé pour la substitution de texte en espéranto
    dans la page principale.

    Étapes d'utilisation :
    1. Importer (ou utiliser par défaut) un **fichier CSV** correspondant
       (racine en espéranto → traduction en français ou caractères chinois).
    2. Importer (ou utiliser par défaut) un **fichier JSON**
       contenant les règles de décomposition des racines et/ou les chaînes à remplacer.
    3. Appuyer sur « Créer le fichier JSON de substitution » pour
       **télécharger** le fichier JSON résultant pour la substitution.

    Des fichiers d'exemple sont disponibles ci-dessous
    comme référence pour le format et pour des modifications personnalisées.
    """)

#=====================================================================
# 3) サンプルファイル一覧 (折りたたみ)
#=====================================================================
with st.expander("Liste de fichiers d'exemple (téléchargement)"):
    st.write("#### Liste des fichiers d'exemple")

    st.markdown("""
    **CSV d'exemple n°1 (Correspondance racines espéranto - traductions françaises avec annotation ruby)**  
    Contient, ligne par ligne, les racines d'espéranto et leur traduction française (avec annotation ruby éventuellement).  
    En téléchargeant un CSV dans ce format, l'outil génère le fichier JSON pour la substitution.
    """)
    file_path0 = './Appの运行に使用する各类文件/Liste des racines de l❜espéranto avec traductions françaises et annotations ruby.csv'
    with open(file_path0, "rb") as file:
        btn = st.download_button(
            label="Télécharger CSV d'exemple n°1 (Racines espéranto - traductions françaises + annotation ruby)",
            data=file,
            file_name="RacinesEspéranto_TraductionsFr_avecAnnotationRuby.csv",
            mime="text/csv"
        )

    st.markdown("""
    **CSV d'exemple n°2 (Correspondance racines espéranto - caractères chinois, proposition de Mingeo)**  
    Il s'agit d'un CSV associant chaque racine d'espéranto à des caractères chinois, selon la proposition de Mingeo sur Zhihu.
    """)
    file_path0 = './Appの运行に使用する各类文件/Mingeo先生版 世界语词根-汉字对应列表.csv'
    with open(file_path0, "rb") as file:
        btn = st.download_button(
            label="Télécharger CSV d'exemple n°2 (Racines espéranto - caractères chinois, version Mingeo)",
            data=file,
            file_name="RacinesEspéranto_CaractèresChinois_Mingeo.csv",
            mime="text/csv"
        )

    st.markdown("""
    **CSV d'exemple n°3 (Correspondance racines espéranto - caractères chinois)**  
    Un autre CSV associant les racines espéranto à des caractères chinois.
    """)
    file_path0 = './Appの运行に使用する各类文件/世界语词根-汉字对应列表.csv'
    with open(file_path0, "rb") as file:
        btn = st.download_button(
            label="Télécharger CSV d'exemple n°3 (Racines espéranto - caractères chinois)",
            data=file,
            file_name="RacinesEspéranto_CaractèresChinois.csv",
            mime="text/csv"
        )

    st.markdown("""
    **JSON d'exemple n°1 (Paramètres utilisateur pour la décomposition des racines espéranto)**  
    **Usage** : précise comment décomposer les mots espéranto en racines,
    à quel moment ajouter un suffixe (par ex. une terminaison verbale), etc.
    Voir les commentaires dans le fichier d'exemple (ex : `["am", "dflt", ["verbo_s1"]]`).
    """)
    ...
    with open(json_file_path, "rb") as file_json:
        btn_json = st.download_button(
            label="Télécharger JSON d'exemple n°1 (Décomposition des racines espéranto)",
            data=file_json,
            file_name="DécompositionRacinesEspéranto_ParamètresUtilisateur.json",
            mime="application/json"
        )

    st.markdown("""
    **JSON d'exemple n°2 (Chaînes de substitution définies par l'utilisateur)**  
    **Usage** : vous pouvez attribuer des caractères chinois ou un format personnalisé
    à certains mots en plus des règles de décomposition de racines ci-dessus.
    (Le plus souvent, il suffit d'éditer le CSV et de se servir du JSON de décomposition, sans besoin de ce fichier.)
    """)
    json_file_path2 = './Appの运行に使用する各类文件/替换后文字列(汉字)の使用者自定义设置(基本上完全不推荐).json'
    with open(json_file_path2, "rb") as file_json:
        btn_json = st.download_button(
            label="Télécharger JSON d'exemple n°2 (Chaînes de substitution définies par l'utilisateur)",
            data=file_json,
            file_name="SubstitutionDéfinieUtilisateur_CaractèresChinois.json",
            mime="application/json"
        )

    st.markdown("""
    **Excel d'exemple n°1 (4736 racines espéranto avec traductions/anotations en 14 langues)**  
    (Japonais, Chinois, Coréen, Anglais, Russe, Espagnol, Italien, Français, Allemand,
    Arabe, Hindi, Polonais, Vietnamien, Indonésien)
    """)
    with open('./Appの运行に使用する各类文件/Kreado de rubenaj komentoj en 14 lingvoj (日本語, 中文, 한국어, English, Русский, español, italiano, français, Deutsch, العربية, हिन्दी, polski, Tiếng Việt, Bahasa Indonesia) respondaj al listo de 4736 Esperant.xlsx', "rb") as file:
        st.download_button(
            label="Télécharger Excel n°1 (Racines espéranto avec traductions dans 14 langues)",
            data=file,
            file_name="RacinesEspéranto_Anotations14Langues.xlsx",
            mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
        )

    st.markdown("""
    **Excel d'exemple n°2 (Racines espéranto - traduction japonaise avec niveau d'apprentissage)**  
    **Usage** : utile si vous souhaitez personnaliser les racines espéranto auxquelles ajouter un rubis de traduction.
    Inclut un « niveau d'apprentissage » basé sur un dictionnaire de base espéranto-japonais.
    """)
    with open('./Appの运行に使用する各类文件/エスペラント語根-日本語訳ルビ対応リスト(習得レベル付き).xlsx', "rb") as file:
        st.download_button(
            label="Télécharger Excel n°2 (Racines espéranto - traduction japonaise avec niveau d'apprentissage)",
            data=file,
            file_name="RacinesEspéranto_JaponaisNiveauApprentissage.xlsx",
            mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
        )
...
#=====================================================================
options = {
    "HTML (avec ajustement de la taille du texte Ruby)": "HTML格式_Ruby文字_大小调整",
    "HTML (avec ajustement de Ruby + substitution de caractères chinois)": "HTML格式_Ruby文字_大小调整_汉字替换",
    "HTML (basique)": "HTML格式",
    "HTML (avec substitution de caractères chinois)": "HTML格式_汉字替换",
    "Format entre parenthèses": "括弧(号)格式",
    "Format entre parenthèses (avec substitution de caractères chinois)": "括弧(号)格式_汉字替换",
    "Substitution simple": "替换后文字列のみ(仅)保留(简单替换)"
}

display_options = list(options.keys())
selected_display = st.selectbox('Sélectionnez le format de sortie :', display_options)
...
st.write("---")
st.markdown("**Texte formaté (exemple) :**")
...
#=====================================================================
st.header("Étape 1 : Préparer le fichier CSV")
st.markdown(
    """
### Veuillez sélectionner le **fichier CSV** contenant les racines de l'espéranto et les informations de traduction (caractères chinois ou français).

---
    """
)

csv_choice = st.radio("Comment procéder avec le fichier CSV ?", ("Importer un fichier CSV", "Utiliser le fichier par défaut"))
csv_path_default = "./Appの运行に使用する各类文件/Liste des racines de l❜espéranto avec traductions françaises et annotations ruby.csv"

CSV_data_imported = None

if csv_choice == "Importer un fichier CSV":
    st.write("Veuillez importer un fichier CSV (UTF-8 recommandé).")
    uploaded_file = st.file_uploader("Sélectionner le fichier CSV", type=['csv'])
    if uploaded_file is not None:
        ...
        st.warning("Aucun fichier CSV n'a été importé.")
        st.stop()

elif csv_choice == "Utiliser le fichier par défaut":
    try:
        ...
        csv_buffer = StringIO(converted_text)
        CSV_data_imported = pd.read_csv(csv_buffer, encoding="utf-8", usecols=[0, 1])
        st.info("Le fichier CSV par défaut va être utilisé.")
    except FileNotFoundError:
        st.error("Impossible de trouver le fichier CSV par défaut. Processus interrompu.")
        st.stop()

st.write("La lecture du fichier CSV est terminée. Passons à l'étape suivante.")
st.write("---")

#=====================================================================
# ステップ2: JSONファイル(語根分解法など)を準備
#=====================================================================
st.header("Étape 2 : Préparer le(s) fichier(s) JSON (règles de décomposition, etc.)")
st.markdown("""
Vous pouvez importer ou utiliser par défaut des fichiers JSON
qui décrivent **les règles de décomposition des racines espéranto** ou
**des chaînes de texte de substitution personnalisées**.
Vous pouvez télécharger un fichier JSON d'exemple et le modifier selon vos besoins.
""")

json_choice = st.radio("1. Comment procéder avec le fichier JSON définissant la décomposition des racines en espéranto ?", ("Importer un fichier JSON", "Utiliser le fichier par défaut"))
json_path_default = "./Appの运行に使用する各类文件/世界语单词词根分解方法の使用者自定义设置.json"
custom_stemming_setting_list = None

if json_choice == "Importer un fichier JSON":
    uploaded_json = st.file_uploader("Importer le fichier JSON", type=['json'])
    if uploaded_json is not None:
        custom_stemming_setting_list = json.load(uploaded_json)
        st.success("Fichier JSON importé avec succès.")
    else:
        st.warning("Aucun fichier JSON n'a été importé.")
        st.stop()
elif json_choice == "Utiliser le fichier par défaut":
    try:
        with open(json_path_default, "r", encoding="utf-8") as g:
            custom_stemming_setting_list = json.load(g)
        st.info("Le fichier JSON par défaut va être utilisé.")
    except FileNotFoundError:
        st.error("Impossible de trouver le fichier JSON par défaut.")
        st.stop()

json_choice2 = st.radio("2. Comment procéder avec le fichier JSON définissant la chaîne de substitution ?", ("Importer un fichier JSON", "Utiliser le fichier par défaut"))
json_path_default2 = "./Appの运行に使用する各类文件/替换后文字列(汉字)の使用者自定义设置(基本上完全不推荐).json"
user_replacement_item_setting_list = None

if json_choice2 == "Importer un fichier JSON":
    uploaded_json = st.file_uploader("Importer le deuxième fichier JSON", type=['json'])
    if uploaded_json is not None:
        ...
        st.warning("Aucun fichier JSON n'a été importé.")
        st.stop()
elif json_choice2 == "Utiliser le fichier par défaut":
    try:
        with open(json_path_default2, "r", encoding="utf-8") as g:
            user_replacement_item_setting_list = json.load(g)
        st.info("Le deuxième fichier JSON par défaut va être utilisé.")
    except FileNotFoundError:
        st.error("Impossible de trouver le deuxième fichier JSON par défaut.")
        st.stop()
...
# ステップ3: 高度な設定 (並列処理)
#=====================================================================
st.header("Étape 3 : Paramètres avancés (traitement parallèle)")
with st.expander("Ouvrir les paramètres concernant le traitement parallèle"):
    st.write("""
    Ici, vous pouvez spécifier le nombre de processus à utiliser
    pour le traitement parallèle lors de la création du fichier JSON.
    Si le texte ou le nombre de racines est très important,
    l'utilisation de plusieurs cœurs CPU peut améliorer la vitesse.
    """)

    use_parallel = st.checkbox("Utiliser le traitement parallèle", value=False)
    num_processes = st.number_input("Nombre de processus simultanés", min_value=2, max_value=6, value=5, step=1)

st.write("### Créer le fichier JSON final pour la substitution (bouton)")

#=====================================================================
# 置換用JSONファイルを作成するボタン
#=====================================================================
if st.button("Créer le fichier JSON pour la substitution"):
    with st.spinner("Génération du fichier JSON de substitution en cours... Veuillez patienter un instant."):
        #-------------------------------------------------------------
        ...
        #     「エスペラント語根 → (汉字 or フランス語訳ルビ等)」を反映させる
        #-------------------------------------------------------------
        ...
        if use_parallel:
            ...
            for i, j in enumerate(E_stem_with_Part_Of_Speech_list):
                ...
                if i % 1000 == 0:
                    ...
                    progress_text.write(f"Traitement de {current_count}/{total_items} éléments...")

            progress_bar.progress(100)
            progress_text.write("La tâche la plus longue est terminée à 100 %. (3~4 secondes supplémentaires sont nécessaires.)")
        ...
        st.success("La génération de la liste de substitution est terminée !")

        st.download_button(
            label="Télécharger la liste finale de substitution (fusion de 3 fichiers JSON)",
            data=download_data,
            file_name="ListeFinaleDeSubstitution_Fusion3FichiersJSON.json",
            mime='application/json'
        )
